...
#endif // ENABLE_WALLET
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("RbcoinMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock, true, NULL))
        return error("RbcoinMiner: ProcessNewBlock, block not accepted");
...
//            }
            ...
            {
                if (GetArg("-mineraddress", "").empty()) {
                    LogPrintf("Error in RbcoinMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                } else {
                    // Should never reach here, because -mineraddress validity is checked in init.cpp
                    LogPrintf("Error in RbcoinMiner: Invalid -mineraddress\n");
                }
                return;
            }
            ...
            LogPrintf("Running RbcoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    catch (const boost::thread_interrupted&)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("RbcoinMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("RbcoinMiner runtime error: %s\n", e.what());
        return;
    }
    miningTimer.stop();
    c.disconnect();
