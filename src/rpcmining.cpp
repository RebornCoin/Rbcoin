...
#else
        throw JSONRPCError(RPC_METHOD_NOT_FOUND, "rbcoind compiled without wallet and -mineraddress not set");
#endif
    }
...
//            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in Satoshis)\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Rbcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Rbcoin is downloading blocks...");
