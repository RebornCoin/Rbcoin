...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Rbcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Rbcoin
    // Mac: ~/Library/Application Support/Rbcoin
    // Unix: ~/.rbcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Rbcoin";
#else
    fs::path pathRet;
...
{
    // Copied from GetDefaultDataDir and adapter for rbcoin params.

    namespace fs = boost::filesystem;
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RbcoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RbcoinParams
    // Mac: ~/Library/Application Support/RbcoinParams
    // Unix: ~/.rbcoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RbcoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "RbcoinParams";
#else
    // Unix
    return pathRet / ".rbcoin-params";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "rbcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "rbcoind.pid"));
...
{
    return "\n" +
           FormatParagraph(strprintf(_("In order to ensure you are adequately protecting your privacy when using Rbcoin, please see <%s>."),
...
std::string LicenseInfo()
{
    return "\n" +
           ...
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The Rbcoin Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
