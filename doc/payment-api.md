...
Rbcoin extends the Bitcoin Core API with new RPC calls to support private Rbcoin payments.

Rbcoin payments make use of two address formats:
...
When transferring funds from one taddr to another taddr, you can use either the existing Bitcoin RPC calls or the new Rbcoin RPC calls.

When a transfer involves zaddrs, you must use the new Rbcoin RPC calls.


## Compatibility with Bitcoin Core

Rbcoin supports all commands in the Bitcoin Core API (as of version 0.11.2).   Where applicable, Rbcoin will extend commands in a backwards-compatible way to enable additional functionality.
...
z_importwallet | filename | _Requires an unlocked wallet or an unencrypted wallet._<br><br>Imports private keys from a file in wallet export file format (see z_exportwallet). These keys will be added to the keys currently in the wallet. This call may need to rescan all or parts of the block chain for transactions affecting the newly-added keys, which may take several minutes.<br><br>Filename is the file to import. The path is relative to rbcoind’s working directory.<br><br>No value is returned but a JSON-RPC error will be reported if a failure occurred.
...
Rbcoin error codes are defined in https://github.com/rbcoin/rbcoin/blob/master/src/rpcprotocol.h
